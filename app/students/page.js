...
export default function StudentDashboard() {
  ...
  const StatusBadge = ({ status }) => {
    // Treat null/undefined/empty status as Fresher
    const normalizedStatus = status ? status.toLowerCase() : "fresher";
    
    const styles = {
      working: "bg-emerald-50 text-emerald-700 border-emerald-200",
      left: "bg-rose-50 text-rose-700 border-rose-200",
      fresher: "bg-blue-50 text-blue-700 border-blue-200",
      default: "bg-gray-50 text-gray-700 border-gray-200"
    };

    const label = normalizedStatus === "fresher" 
      ? "Fresher" 
      : normalizedStatus.charAt(0).toUpperCase() + normalizedStatus.slice(1);

    return (
      <span className={`px-3 py-1 rounded-full text-xs font-medium border ${styles[normalizedStatus] || styles.default}`}>
        {label}
      </span>
    );
  };
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          ...
          <div className="p-4 border-b border-gray-200 flex flex-col lg:flex-row items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3 w-full lg:w-auto justify-end">
              ...
              <Link href={'/add-students'} title="add students" className="flex items-center gap-2 px-4 py-2 bg-[#7F56D9] text-white rounded-lg text-sm font-medium hover:bg-[#6941C6] transition-colors shadow-sm">
                + Add Student
              </Link>
            </div>
          </div>
                    ...
                    <tr key={student._id || index} className="hover:bg-gray-50/80 transition-colors group">
                      ...
                      <td className="p-4">
                        <div className="flex items-center gap-3">
                          <div className="w-9 h-9 rounded-full bg-[#7F56D9]/10 text-[#7F56D9] flex items-center justify-center font-bold text-xs shrink-0 group-hover:bg-[#7F56D9] group-hover:text-white transition-colors">
